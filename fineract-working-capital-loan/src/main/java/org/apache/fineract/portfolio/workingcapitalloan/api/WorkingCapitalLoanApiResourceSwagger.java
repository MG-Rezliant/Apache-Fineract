...
public final class WorkingCapitalLoanApiResourceSwagger {
    ...
    public static final class GetWorkingCapitalLoansLoanIdResponse {
        ...
        @Schema(description = "Period payment rate change history, most recently booked first - which for a backdated change is not "
                + "the same as effective-date order. Each entry carries the EIR and the other values the amortization schedule "
                + "computed when that change was booked; those are null for changes booked before the snapshot was introduced")
