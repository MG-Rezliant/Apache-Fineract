...
* `WorkingCapitalLoanTransactionReversedBusinessEvent` — emitted alongside the undo event, mirroring the other transaction reversals, so a consumer reconciling reversals generically does not miss recovery ones.
